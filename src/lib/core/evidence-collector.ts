...
const STEP_PAUSE_MS = Number(process.env.P3_STEP_PAUSE_MS ?? "100") || 100;
...
export async function collectEvidenceDeterministic(options: {
  ...
  if (options.orderId.startsWith('NL-')) options = { ...options, tools: [...new Set<InvestigationToolName>([...options.tools, 'getOrderDetails', 'getPaymentDetails', 'getCustomerHistory', 'getRefundHistory', 'getFulfillmentDetails', 'getTrackingStatus'])] };
