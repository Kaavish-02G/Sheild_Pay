...
export function buildEvidenceOnlyResponse(evidence: VerifiedEvidencePackage): string {
  ...
  return (
    `${evidence.disputeId.startsWith("sim-") ? "DEMO RESPONSE — no live payment or bank submission. " : ""}This response concerns order ${orderRef}. ` +
    `Our verified records show the following: ${facts.join("; ")}. ` +
    `These records establish only the facts listed above; delivery, authorization, and receipt must not be inferred where supporting proof is absent. ` +
    `Please assess the dispute using these records and the outstanding evidence requirements. Merchant review is required when evidence is incomplete.`
  );
}
